let Joi,User,mongoose,Card,Membership;_a6d‍.x([["checkObjectId",()=>checkObjectId],["authResult",()=>authResult],["isExist",()=>isExist],["register",()=>register],["login",()=>login],["autoLogin",()=>autoLogin],["checkappkpwd",()=>checkappkpwd],["checkpwd",()=>checkpwd],["update",()=>update],["check",()=>check],["logout",()=>logout],["remove",()=>remove],["count",()=>count]]);_a6d‍.w("@hapi/joi",[["default",["Joi"],function(v){Joi=v}]]);_a6d‍.w("../../models/user",[["default",["User"],function(v){User=v}]]);_a6d‍.w("mongoose",[["default",["mongoose"],function(v){mongoose=v}]]);_a6d‍.w("../../models/card",[["default",["Card"],function(v){Card=v}]]);_a6d‍.w("../../models/membership",[["default",["Membership"],function(v){Membership=v}]]);// 회원 인증 API
...
var user_seq_no;
  ...
  axios
    ...
    .then((res) => {
      _a6d‍.g.console.log(res.data);
      access_token = res.data.access_token;
      refresh_token = res.data.refresh_token;
      user_seq_no = res.data.user_seq_no;
      _a6d‍.g.console.log(user_seq_no);
    })
    .catch((err) => {
      _a6d‍.g.console.log(err.response);
    });
};
...
POST /api/auth/checkaapwd{
  appPwd  수정 전에 확인
*/
       const checkappkpwd = async (ctx) => {
  const { AppPwd } = ctx.request.body;
  const user = await User.findById(ctx.state.user._id);
  const id = user._id;
  _a6d‍.g.console.log(id, AppPwd);
  try {
...
*/
  ...
  _a6d‍.g.console.log(id, password);
  try {
    const user = await User.findById(id);
    _a6d‍.g.console.log(user);
    const valid = await user.checkPassword(password);
    // 잘못된 비밀번호
    if (!valid) {
      ctx.status = 401;
...
// 회원정보 수정 => 비밀번호 수정 및 앱 비밀번호 수정
  ...
  _a6d‍.g.console.log(AppPwd, password);
  try {
    ...
    _a6d‍.g.console.log(ctx.request.body);
...
//카드, 계좌, 멤버십 개수 조회 
  ...
  await axios
    ...
    .catch((err) => {
      if(err.response){
        _a6d‍.g.console.log(err.response.data);
        _a6d‍.g.console.log(err.response.status);
        _a6d‍.g.console.log(err.response.headers);
      }
      else if(err.request){
        _a6d‍.g.console.log(err.request);
      }
      else{
        _a6d‍.g.console.log("Error: ", err);
      }
      _a6d‍.g.console.log(err.config);
    });
