let Joi,Account,User,mongoose,read;_29a‍.x([["list",()=>list],["balance",()=>balance],["transactionList",()=>transactionList],["transfer",()=>transfer],["transactionAll",()=>transactionAll],["transactionGraph",()=>transactionGraph]]);_29a‍.w("@hapi/joi",[["default",["Joi"],function(v){Joi=v}]]);_29a‍.w("../../models/account",[["default",["Account"],function(v){Account=v}]]);_29a‍.w("../../models/user",[["default",["User"],function(v){User=v}]]);_29a‍.w("mongoose",[["default",["mongoose"],function(v){mongoose=v}]]);_29a‍.w("../card/cards.ctrl",[["read",["read"],function(v){read=v}]]);// 계좌 인증 API
...
//계좌 목록 조회 (testapi에서 받아온 값을 바로 body로 전송)
  ...
  await axios
    .get(url, config)
    .then((res) => {
      _29a‍.g.console.log(res.data);
      ctx.body = res.data.res_list;
    })
    .catch((err) => {
      if(err.response){
        _29a‍.g.console.log(err.response.data);
        _29a‍.g.console.log(err.response.status);
        _29a‍.g.console.log(err.response.headers);
      }
      else if(err.request){
        _29a‍.g.console.log(err.request);
      }
      else{
        _29a‍.g.console.log("Error: ", err);
      }
      _29a‍.g.console.log(err.config);
    });
};
...
//거래내역 조회
  ...
  _29a‍.g.console.log(fintech_use_num);
  ...
  await axios
    ...
    .catch((err) => {
      _29a‍.g.console.log(err.response);
    });
};
...
//통합내역조회
       const transactionAll = async (ctx) => {
...
//통합내역조회(그래프용). 출금만 조회
       const transactionGraph = async (ctx) => {
  const {from_date, to_date} = ctx.query;
  const user = await User.findById(ctx.state.user._id);
