let Card,Joi,mongoose;_07e‍.x([["getCardById",()=>getCardById],["add",()=>add],["list",()=>list],["read",()=>read],["remove",()=>remove]]);_07e‍.w("../../models/card",[["default",["Card"],function(v){Card=v}]]);_07e‍.w("@hapi/joi",[["default",["Joi"],function(v){Joi=v}]]);_07e‍.w("mongoose",[["default",["mongoose"],function(v){mongoose=v}]]);



const { ObjectId } = mongoose.Types;

// ObjectId 검증하기 => update에서 사용
       const getCardById = async (ctx, next) => {
  const { id } = ctx.params;
  if (!ObjectId.isValid(id)) {
    ctx.status = 400; //Bad Request
    return;
  }
  try {
    const card = await Card.findById(id);
    //멤버쉽이 존재하지 않을 때
    if (!Card) {
      ctx.status = 404; // Not Found
      return;
    }
    ctx.state.membership = card;
    return next();
  } catch (e) {
    ctx.throw(500, e);
  }
};

/*
POST /api/cards
{
    Cardnumber_d: 앞 4자리
  Cardnumber: 뒷 12자리 숫자, // 사용자 카드번호,
  validity: 4자리 숫자, //유효기간 MMYY,
  Cardcvc: 3자리 숫자, // 사용자 카드cvc,
  CardPassword: 2자리 숫자,// 사용자 카드비밀번호 앞 두자리
  AppPwd: 6자리 숫자, // 사용자 앱 비밀번호
}
*/
//카드 등록
       const add = async (ctx) => {
  // Requeset Body 검증
  const schema = Joi.object().keys({
    bank: Joi.string().required(),
    cardNumber_d: Joi.string().length(4).required(),
    cardNumber: Joi.string().length(12).required(),
    validity: Joi.string().length(4).required(),
    cardCvc: Joi.string().length(3).required(),
    cardPassword: Joi.string().length(4).required(),
  });
  // 양식이 맞지 않으면 400 에러
  const result = schema.validate(ctx.request.body);
  if (result.error) {
    ctx.status = 400;
    ctx.body = result.error;
    return;
  }
  const {
    bank,
    cardNumber_d,
    cardNumber,
    validity,
    cardCvc,
    cardPassword,
  } = ctx.request.body;

  try {
    const card = new Card({
      bank,
      cardNumber_d,
      validity,
      user: ctx.state.user,
    });
    await card.setCard(cardNumber, cardCvc, cardPassword); // 카드 정보 설정
    await card.save(); // 데이터베이스에 저장

    // 응답할 데이터에서 hashed 필드 제거
    ctx.body = card.serialize();
  } catch (e) {
    ctx.throw(500, e);
  }
};

/*
GET /api/cards
*/
// 멤버쉽카드 리스트 조회

       const list = async (ctx) => {
  try {
    const cards = await Card.find.exec();
    ctx.body = cards;
  } catch (e) {
    ctx.throw(500, e);
  }
};

/*
GET /api/cards/:id
*/
// 특정 카드 조회
       const read = (ctx) => {
  ctx.body = ctx.state.card;
};

/*
  DELETE /api/cards/:id
  */
// 특정 카드 삭제
       const remove = async (ctx) => {
  const { id } = ctx.params;
  try {
    await Card.findByIdAndRemove(id).exec();
    ctx.status = 204; // No Content(성공하기는 했지만 응답할 데이터는 없음)
  } catch (e) {
    ctx.throw(500, e);
  }
};
